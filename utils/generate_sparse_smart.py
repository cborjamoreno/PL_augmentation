...
def generate_smart_points(mask_generator,img, num_labels):
    # use Segment Anything to do a first segmentation
    ...
    for mask in masks:
        ...
        centroids.append((centroid, mask['area']))

    # Sort centroids by area
    centroids = sorted(centroids, key=lambda x: x[1], reverse=True)

    # Select points based on the number of masks
    if len(centroids) >= num_labels:
        selected_points = [centroid for centroid, _ in centroids[:num_labels]]
    else:
        selected_points = [centroid for centroid, _ in centroids]
        # Add additional points randomly if needed
