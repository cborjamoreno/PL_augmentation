        ...
        if count == 0 or len(selected_points_and_labels) < num_labels:
            ...
            if len(cell_indices) > 0:
                ...
                if best_point is None:
                    ...
                    best_point = (tuple(cell_indices[random.randint(0, len(cell_indices) - 1)]), None)
                    # print(f"Selected random point {best_point[0]} in cell ({i}, {j}) after {max_attempts} attempts.")
                ...
                # Plot the points and the grid after adding each grid point
    # plt.figure(figsize=(10, 10))
    # plt.imshow(img)
    # for idx, ((x, y), _) in enumerate(selected_points_and_labels):
    #     color = 'red' if idx < initial_points_count else 'blue'
    #     plt.scatter(y, x, c=color, marker='o')  # Note: (y, x) for plotting
    # for k in range(grid_size + 1):
    #     plt.axhline(k * grid_height, color='blue', linestyle='--')
    #     plt.axvline(k * grid_width, color='blue', linestyle='--')
    # plt.title(f'Selected Points and Grid (After Adding Point {random_point})')
    # plt.xlabel('X Coordinate')
    # plt.ylabel('Y Coordinate')
    # plt.grid(True)
    # plt.show()
