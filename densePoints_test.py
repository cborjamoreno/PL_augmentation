...
class SAMLabelExpander(LabelExpander):
    def __init__(self, color_dict, labels, output_df, predictor, eval_images=False):
    ...
    def expand_labels(self, points, labels, unique_labels, unique_labels_str, image, image_name, background_class=None, eval_image_dir=None):
        ...
        for idx, (_points, _labels) in enumerate(zip(filtered_points_all, filtered_labels_all)):
            ...
            for p, l in zip(_points_pred, _labels_ones):
                ...
                # Print the three masks together
                # fig, axes = plt.subplots(1, 3, figsize=(15, 5))
                # for i, (ax, score) in enumerate(zip(axes, scores)):
                #     binary_mask = masks[i].astype(np.uint8) * 255
                #     ax.imshow(binary_mask, cmap='gray')
                #     if i == selected_mask:
                #         ax.set_title(f'Segment {segment_counter} - Mask {i+1} - Score: {score:.4f} (Selected)', color='red')
                #     else:
                #         ax.set_title(f'Segment {segment_counter} - Mask {i+1} - Score: {score:.4f}')
                #     ax.axis('off')
                # plt.show()
        ...
        del expanded_df, new_data_df, gt_points, gt_labels
...
LabelExpander_sam = SAMLabelExpander(color_dict, unique_labels, output_df, predictor, eval_images)
